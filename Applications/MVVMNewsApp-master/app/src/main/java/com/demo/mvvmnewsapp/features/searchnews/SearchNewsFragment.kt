...
import com.demo.mvvmnewsapp.MainActivity
import com.demo.mvvmnewsapp.R
import com.demo.mvvmnewsapp.databinding.FragmentSearchNewsBinding
import com.demo.mvvmnewsapp.util.onQueryTextSubmit
import com.demo.mvvmnewsapp.util.showIfOrInvisible
        ...
        binding.apply {
            recyclerView.apply {
                adapter = newsArticleAdapter.withLoadStateFooter(
                    /**
                     * We pass the retry function to the adapter here
                     * * newsArticleAdapter::retry ---> This is called function reference
                     */
                    NewsArticleLoadStateAdapter(newsArticleAdapter::retry)
                )
                layoutManager = LinearLayoutManager(requireContext())
                setHasFixedSize(true)
                // Avoiding blink effect after refresh or after selecting a book-mark
                itemAnimator?.changeDuration = 0
            }

            viewLifecycleOwner.lifecycleScope.launchWhenStarted {
                /**
                 *  Here we collect the search results
                 *  collectLatest --> This here ensures when the new invocation is happened, previous one is canceled and new one is used
                 */
                viewModel.searchResults.collectLatest { data ->
                    /**
                     * Add the new data to the adapter
                     * SubmitData suspends until data is displayed
                     * */
                    ...
                    .collect { loadState ->
                        when (val refresh = loadState.mediator?.refresh) {
                            ...
                            is LoadState.NotLoading -> {
                                textViewError.isVisible = false
                                false.also { buttonRetry.isVisible = it }
